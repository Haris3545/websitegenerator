 ...
 * at creation is fine.
 *
 * This used to also run a "warming" phase here that hit every enabled tab's
 * real page before handing off to the site — moved to the site itself (see
 * SiteWarmupOverlay, triggered via a `?warming=1` flag on the redirect in
 * ArtistForm) after that in-builder version reportedly wasn't showing up at
 * all for a real user despite working in every code review and local
 * reproduction here — putting it on the page that's actually guaranteed to
 * render, rather than continuing to debug an overlay this component could
 * never independently confirm was mounting in that environment. */
