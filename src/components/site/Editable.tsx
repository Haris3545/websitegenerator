...
/** Wraps a piece of static site copy so, while "Edit page" mode is on (see
 * SiteFooter), clicking it opens an inline editor; saving writes a permanent
 * per-artist override (see updateContentOverride) instead of just changing
 * this one page view. Outside edit mode this renders as plain, inert text —
 * normal clicks (including nav tabs) behave exactly as they always did.
...
export function Editable({
  ...
  return (
    <Component
      ...
      title="Click to edit"
      className={`${className} ${isPending ? "opacity-50" : ""} cursor-text rounded outline outline-1 outline-dashed outline-[var(--accent)]/60 hover:bg-[var(--accent)]/10`}
    >
      {current}
    </Component>
  );
