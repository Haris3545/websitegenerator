...
export default async function GatePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: artist } = await supabase
    .from("artists")
    .select(
      "id, accent_color, gate_background_url, gate_youtube_id, gate_youtube_start, gate_youtube_end, gate_scrim_opacity, gate_grain_intensity, gate_grain_monochrome, project_title, tagline, font_family"
    )
    .eq("slug", slug)
    .maybeSingle();
